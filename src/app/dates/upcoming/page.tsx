...
const UpcomingDatesPage: FC = () => {
  ...
  const fetchUpcomingDates = async () => {
    try {
      ...
      // Fetch dates where user is receiver
      const { data: receiverDates } = await supabase
        .from('date_requests')
        .select(
          `
          id,
      ...
      // Fetch dates where user is sender
      const { data: senderDates } = await supabase
        .from('date_requests')
        .select(
          `
          id,
      ...
      // Format receiver dates
      const formattedReceiverDates: UpcomingDate[] = (receiverDates || []).map(
        (date) => ({
          id: date.id,
      ...
      // Format sender dates
      const formattedSenderDates: UpcomingDate[] = (senderDates || []).map(
        (date) => ({
          id: date.id,
          otherPerson: date.profiles,
          venue: date.venue,
      ...
      const allDates = [
        ...formattedReceiverDates,
        ...formattedSenderDates,
      ].sort(
        (a, b) =>
            ...
            <div className='flex items-center mb-4'>
              ...
              <div>
                <h3 className='text-[#cc0000] text-xl font-medium mb-1'>
                  {date.otherPerson.first_name} {date.otherPerson.last_name},{' '}
                  {date.otherPerson.age}
                </h3>
                <p className='text-gray-600 mb-1'>{date.otherPerson.bio}</p>
                <p className='mb-1 font-medium'>
                  When: {new Date(date.proposed_time).toLocaleDateString()},{' '}
                  {new Date(date.proposed_time).toLocaleTimeString()}
                </p>
                <p className='font-medium'>Where: {date.venue}</p>
                {date.proposed_payment > 0 && (
                  <p className='font-medium text-green-600'>
                    {date.isSender ? 'Offering: ' : 'Payment: '}$
                    {date.proposed_payment}
                  </p>
                )}
              </div>
            </div>
