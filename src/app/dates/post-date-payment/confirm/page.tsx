...
export default function ConfirmBill() {
    ...
    useEffect(() => {
        console.log(searchParams.has('amount'));
        if (searchParams.has('amount')) {
            setManual(true);
            setManualAmount(parseFloat(searchParams.get('amount')!));
        }
        ...
        const fetchClientSecret = async () => {
            try {
                const response = await fetch('/api/create-payment-intent', {
                    ...
                    body: JSON.stringify({ amount: searchParams.has('amount') ? parseFloat(searchParams.get('amount')!) * 0.15 : receipt?.opheliaFee}),
                });
            ...
            <h2 className="text-xl font-bold mb-4">Confirm Your Bill</h2>
                { !manual ? (
                    ...
                    <div className="flex justify-between items-center px-2 py-1 mb-6">
                        ...
                        <div className="flex items-center space-x-2 w-1/5">
                            <span className="flex-1 text-right font-medium">$</span> {/* Fixed width for price column */}
                            <span className="w-20 text-right font-medium">{manualAmount?.toFixed(2)}</span> {/* Fixed width for price column */}
                        </div>
                    </div>
                )}
