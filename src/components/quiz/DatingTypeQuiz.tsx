...
const DatingTypeQuiz: React.FC<DatingTypeQuizProps> = ({ onComplete }) => {
  ...
  const determineDatingStyle = () => {
    // Map answers to dating styles based on the most common answer pattern
    const styleScores = {
      "Hopeless Romantic": 0,
      "Serial Dater": 0,
      "Commitment Seeker": 0,
    ...
    // Analyze answers and increment scores
    Object.values(answers).forEach((answer) => {
      switch(answer) {
        case 0: // Romantic choices
          styleScores["Hopeless Romantic"] += 1
          break
        case 1: // Adventurous choices
          styleScores["Serial Dater"] += 1
          break
        case 2: // Laid-back choices
          styleScores["Friends with Benefits"] += 1
          break
        case 3: // Thoughtful choices
    ...
    let maxScore = 0
    let resultStyle = "Cautious Dater" // default style
