...
interface PaymentPageProps {
  selectedDate: DateType  // Changed from 'date' to 'selectedDate'
  onConfirm: () => void
  onCancel: () => void
}

const PaymentPage: React.FC<PaymentPageProps> = ({ selectedDate, onConfirm, onCancel }) => {
  ...
  return (
    <div className="max-w-md mx-auto p-5">
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-6">
        ...
        <p className="mb-1">
          <strong>With:</strong> {selectedDate.name}, {selectedDate.age}
        </p>
        <p className="mb-1">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button 
          ...
          className={`w-full p-3 bg-[#cc0000] text-white text-lg rounded-full cursor-pointer font-medium transition-colors mt-6 
      ...
      <button 
        className="w-full p-3 mt-4 bg-white text-[#cc0000] border-2 border-[#cc0000] rounded-full cursor-pointer font-medium hover:bg-[#ffeeee] transition-colors"
        onClick={onCancel}
        disabled={isProcessing}
      >
        Cancel
